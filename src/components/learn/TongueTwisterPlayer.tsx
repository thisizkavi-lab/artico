                    ...
                    {(["easy", "medium", "hard"] as const).map((difficulty) => (
                        <div key={difficulty} className="mb-8">
                            <h2 className="text-lg font-semibold mb-4 text-white">
                                {difficultyLabels[difficulty]} {difficulty.charAt(0).toUpperCase() + difficulty.slice(1)}
                            </h2>
                                ...
                                {tongueTwisters
                                    ...
                                    .map((twister) => (
                                        <button
                                            ...
                                            className="w-full flex items-center gap-4 p-4 bg-ash hover:bg-cocoa rounded-xl transition-all group"
                                        >
                                            <div className="w-12 h-12 bg-cocoa rounded-lg flex items-center justify-center text-2xl">
                                                🎤
                                            </div>
                                            ...
                                            <svg className="w-5 h-5 text-dust group-hover:text-white transition-colors" viewBox="0 0 24 24" fill="currentColor">
        ...
        <div className="fixed inset-0 z-50 bg-ink flex flex-col">
            ...
            <div className="flex items-center justify-between px-6 py-4">
                <button
                    ...
                    className="text-dust hover:text-white transition-colors flex items-center gap-2"
                >
            ...
            <div className="flex-1 flex flex-col items-center justify-center px-6">
                ...
                <div className="text-center mb-8">
                    ...
                    <p className="text-dust">
                        {difficultyLabels[selectedTwister.difficulty]} {selectedTwister.difficulty} • Focus: {selectedTwister.focusSound}
                    </p>
                </div>

                {/* Lyrics display */}
                <div className="bg-ash rounded-2xl p-8 w-full max-w-xl mb-8">
                                    ...
                                    return (
                                        <span
                                            ...
                                            className={`transition-all duration-150 ${isActive
                                                ? "text-white font-bold scale-110 inline-block"
                                                : isPast
                                                    ? "text-pebble"
                                                    : "text-dust"
                                                }`}
                    ...
                    {mode === "shadow" && shadowPhase === "repeat" && (
                        <div className="mt-6 text-center">
                            <p className="text-cocoa animate-pulse text-lg font-medium">
                ...
                <div className="w-full max-w-xl mb-6">
                    <div className="flex items-center gap-4">
                        <span className="text-xs text-dust w-10">
                            {Math.floor(currentTime / 1000)}s
                        </span>
                        <div className="flex-1 h-1.5 bg-ash rounded-full overflow-hidden">
                            <div
                                className="h-full bg-cocoa rounded-full transition-all"
                                style={{ width: `${Math.min(progress, 100)}%` }}
                            />
                        </div>
                        <span className="text-xs text-dust w-10 text-right">
                ...
                <div className="flex items-center gap-4 mb-8">
                    <span className="text-dust text-sm">🐢</span>
                ...
                <button
                    onClick={togglePlay}
                    className="w-16 h-16 bg-cocoa rounded-full flex items-center justify-center hover:scale-105 transition-transform shadow-lg mb-8"
                >
                    {isPlaying ? (
                    ...
                    {(["listen", "shadow", "practice"] as const).map((m) => (
                        <button
                            ...
                            className={`px-5 py-2.5 rounded-full font-medium transition-all ${mode === m
                                ? "bg-white text-ink"
                                : "bg-ash text-white hover:bg-cocoa"
                                }`}
                        >
                            {m === "listen" && "🎧 Listen"}
                            {m === "shadow" && "🎤 Shadow"}
                ...
                {mode === "practice" && (
                    <div className="mt-6 text-center">
                        {!audioUrl ? (
                            <>
                                <p className="text-dust text-sm mb-3">
                                ...
                                <button
                                    ...
                                    className={`px-6 py-3 rounded-full font-medium transition-all flex items-center gap-2 mx-auto ${isRecording
                                        ? "bg-cocoa text-white animate-pulse"
                                        : "bg-ash text-pebble hover:bg-cocoa hover:text-white"
                                        }`}
                                >
                                    {isRecording ? (
                                        <>
                            ...
                            <div className="flex flex-col items-center gap-4">
                                <p className="text-pebble font-medium">Recording saved!</p>
                                <div className="flex gap-3">
                                    <button
                                        onClick={() => {
                                        ...
                                        className="px-5 py-2.5 bg-ash hover:bg-cocoa rounded-full text-white font-medium flex items-center gap-2"
                                    >
                                        ▶️ Play My Recording
                                    </button>
                                    <button
                                        onClick={() => setAudioUrl(null)}
                                        className="px-5 py-2.5 bg-ash hover:bg-cocoa rounded-full text-dust hover:text-white font-medium"
                                    >
                                        Try Again
                                    </button>
                                </div>
